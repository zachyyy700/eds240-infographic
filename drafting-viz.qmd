...
japan <- read_sf(here::here('data', 'jp.json'))
```
...
ak_winners <- ggplot(arima_kinen_all) + 
...
# # Save Arima Kinen Winners as png
# ggsave(here::here("outputs", paste0("ak_winners_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".png")), dpi = 320, width = 9, height = 10)

# Save Arima Kinen Winners as vector
# ggsave(here::here("outputs", paste0("ak_winners_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".pdf")), width = 9, height = 10)
```
...
```{r}
# showtext_opts(dpi = 320)
# # Save Arima Kinen times as vector
# ggsave(here::here("outputs", paste0("ak_times_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".png")), dpi = 320, width = 9, height = 6)
```
...
```{r}
# showtext_opts(dpi = 320)
# Export AK prize as vector
# ggsave(here::here("outputs", paste0("ak_prize_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".png")), dpi = 320, width = 9, height = 6)
```
...
ak_fav_winrates <- arima_kinen_all |> 
    mutate(win_fav_bin = case_when(
        ...
        win_fav %in% c(10, 11, 12) ~ "10-12",
        win_fav %in% c(13, 14, 15, 16) ~ "13-16"
    ), .after = win_fav) |> 

    mutate(win_fav_bin = factor(win_fav_bin, levels = c("1-3", "4-6", "7-9", "10-12", "13-16")))
```
...
ak_fav_winrates |> 
    ...
    mutate(win_rate_label = label_percent()(win_rate)) |> 
    ggdonutchart(x = 'win_rate', label = 'win_rate_label',
    color = 'white',
    fill = 'win_fav_bin',
    palette = c(colors[['deepred']], colors[['orange']], colors[['yellow']], colors[['blue']], colors[['green']])) +

    labs(title = "Arima Kinen Winrates by Favorite Rank", fill = 'Favorite Rank') +
...
```{r}
# ggsave(here::here("outputs", paste0("ak_winrates_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".pdf")), width = 9, height = 6)
```

# AK Elite Competitors
...
ggplot(ak_champs, aes(x = grade_winpct, y = factor(horse_name_eng, levels = champ_order), fill = race_grade)) +
    geom_col(position = position_dodge(reverse = TRUE)) +
    geom_text(aes(label = horse_name_eng, x = 0.01), hjust = 0, size = 7, color = colors[['tan']]) +
    labs(x = "Win Percentage", fill = "Race Grade") +
...
```{r}
# showtext_opts(dpi = 320)
# ggsave(here::here("outputs", paste0("ak_top10_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".png")), width = 9, height = 6)
```

# Geographic Context

```{r}
japan_combined <- st_union(japan)
print(st_crs(japan_combined))
```

```{r}
nakayama_course <- tribble(
    ~lon, ~lat,
    139.9614677837383, 35.72498856698089
)

nakayama_sf <- st_as_sf(nakayama_course, coords = c('lon', 'lat'), crs = st_crs(japan))
```
```{r fig.width=6, fig.height=6}
ggplot() +
    geom_sf(data = japan_combined, fill = NA) +
    geom_sf(data = japan |> filter(name == "Chiba"), fill = NA, color = 'red') +
    geom_sf(data = nakayama_sf) +
    theme_void()
```

```{r}
# ggsave(here::here("outputs", paste0("map_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".pdf")), width = 6, height = 6)
```
