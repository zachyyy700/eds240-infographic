...
```{r}
ggplot(ak_margins_all, aes(margin_parsed)) +
    geom_histogram() +
    theme_minimal()
```
...
ak_winners <- jra_results_eng |> 
    filter(race_name == "有馬記念", final_position == 1) |> 
    select(race_id, race_date, distance_m, horse_number, horse_name, horse_age, finish_time, final_600m_time, win_odds_100Y, popular_rank, prize_money_10kY, year, horse_trainer, horse_owner) 
```
...
```{r}
# Combine age dfs
age_combined <- jra_age_dist |> 
    select(horse_age, jra_perc = age_perc) |> 
    full_join(ak_age_dist |> select(horse_age, ak_perc = age_perc), by = 'horse_age') |> 
...
ggplot(data = age_combined) +
    ...
    scale_color_manual(values = c("All Races" = 'grey59', "Arima Kinen" = colors[1])) +
...
ggplot(prize_g1_all) +
    ...
    scale_color_manual(values = c(colors[1], colors[3], colors[5])) +
...
ak_fav_winrates <- arima_kinen_all |> 
    mutate(popular_rank_bin = case_when(
        popular_rank %in% c(1, 2, 3) ~ "1-3",
        popular_rank %in% c(4, 5, 6) ~ "4-6",
        popular_rank %in% c(7, 8, 9) ~ "7-9",
        popular_rank >= 10 ~ "10+"
    ), .after = popular_rank) |> 

    mutate(popular_rank_bin = factor(popular_rank_bin, levels = c("1-3", "4-6", "7-9", "10+")))
```

```{r}
ak_fav_winrates |> 
    group_by(popular_rank_bin) |> 
    summarise(win_rate = n() / nrow(ak_fav_winrates)) |> 
    ggdonutchart(x = 'win_rate', label = 'popular_rank_bin', lab.pos = 'in',
    color = 'white',
    fill = 'popular_rank_bin',
    palette = c(colors[3], colors[4], colors[5], colors[6])) +
