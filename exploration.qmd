...
### Effect of track condition on performance

```{r}
course_conditions <- jra_results_eng |> 
    filter(final_position == 1) |> 
    group_by(racecourse_name, distance_m, track_condition1) |> 
    summarise(avg_time = mean(finish_time, na.rm = TRUE), races = n()) |> 
    ungroup() |> 
    mutate(track_condition1 = factor(track_condition1, levels = c("良", "稍重", "重", "不良")))

filtered_conditions <- course_conditions |> 
    filter(distance_m %in% c(1200, 1600, 1800, 2000), racecourse_name %in% c("中京", "東京", "中山", "京都", "阪神"))


ggplot(filtered_conditions, aes(x = track_condition1, y = avg_time)) +
    geom_jitter() +
    facet_grid(racecourse_name ~ distance_m)
```
...
wincount_df <- jra_results_eng |> 
    filter(final_position == 1) |> 
    group_by(horse_name) |> 
    summarise(total_wins = sum(final_position, na.rm = TRUE)) |> 
...
```{r}
# Select famous horses
hof_list <- c("トウカイテイオー", "オグリキャップ", "メジロマックイーン", "ナリタブライアン", "タイキシャトル", "テイエムオペラオー", "ディープインパクト", "ウオッカ", "エルコンドルパサー", "オルフェーヴル", "ジェンティルドンナ", "ロードカナロア", "キタサンブラック", "アーモンドアイ", "コントレイル", "キングカメハメハ", "スペシャルウィーク", "シンボリクリスエス")
# Distance specialization
distance_specialize <- jra_results_eng |> 
    mutate(won = ifelse(final_position == 1, yes = 1, no = 0)) |> 
    group_by(horse_name, distance_category) |> 
    summarise(wins = sum(won, na.rm = TRUE), races = n(), .groups = 'drop') |> 

    mutate(win_rate = wins / races) |> 
    arrange(desc(win_rate)) |> 
    filter(horse_name %in% hof_list)
```

```{r}
# Distance wins proportion chart
ggplot(distance_specialize, aes(x = wins, y = horse_name, fill = distance_category)) +
...
# Pace
jra_results_long |> 
    filter(race_name == "有馬記念" & year == 1990) |> 
    ggplot(aes(x = corner_number, y = factor(position), group = horse_name, color = horse_name)) +
    geom_line(linewidth = 1.5) +
    ...
    #Highlight winner
    gghighlight(horse_name == "オグリキャップ", 
    label_params = list(
        nudge_x = -1.5, nudge_y = 1
    ), 
    use_group_by = FALSE) +
...
# End
jra_results_long |> 
    filter(race_name == "有馬記念" & year == 2012) |> 
    ggplot(aes(x = corner_number, y = factor(position), group = horse_name, color = horse_name)) +
    geom_line(linewidth = 1.5) +
    ...
    # Highlight winner
    gghighlight(horse_name == "ゴールドシップ", 
    label_params = list(
        nudge_x = -1.5, nudge_y = 1
    ), 
    use_group_by = FALSE) +
...
```{r}
# Number one favorite winrate
all_arima_kinen |> 
    filter(popular_rank == 1) |> 
    summarise(win_rate = mean(final_position == 1, na.rm = TRUE))

# Number two favorite winrate
all_arima_kinen |> 
    filter(popular_rank == 2) |> 
    summarise(win_rate = mean(final_position == 1, na.rm = TRUE))

# Number three favorite winrate
all_arima_kinen |> 
...
find_winrates <- function(data, race, graded = "G1"){
    # Filter to race
    race_only <- data |> 
